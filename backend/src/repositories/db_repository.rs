...
mod tests {
	...
	async fn test_crud_scenario() {
		...
		let created = repository.create(user_id, content_1.to_string())
			.await
		...
		// --- Test B: ID検索 (Read) ---
		let found = repository.find_by_id(user_id)
			.await
		...
		let updated = repository.create(user_id, content_2.to_string())
			.await
			.expect("Failed to update calligraphy");

		assert_eq!(updated.content, content_2);
		assert!(updated.updated_at > created.updated_at, "Updated time should be newer");
		println!("Test C Passed: Updated content");

		// --- Test D: 一覧取得 (Find All) ---
		let list = repository.find_all()
			.await
